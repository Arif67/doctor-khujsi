...
<div class="bg-white shadow rounded-lg p-6">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-semibold text-gray-800">Edit District</h2>
    ...
    <form action="{{ route('admin.districts.update', $district->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block font-semibold mb-1">Name</label>
                <input type="text" name="name" value="{{ old('name', $district->name) }}" class="w-full border px-3 py-2 rounded" required>
                @error('name') <p class="text-red-500 font-semibold">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block font-semibold mb-1">Bangla Name</label>
                <input type="text" name="bn_name" value="{{ old('bn_name', $district->bn_name) }}" class="w-full border px-3 py-2 rounded">
            ...
            <div>
                ...
                <input type="number" name="division_id" value="{{ old('division_id', $district->division_id) }}" class="w-full border px-3 py-2 rounded">
                @error('division_id') <p class="text-red-500 font-semibold">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block font-semibold mb-1">URL</label>
                <input type="text" name="url" value="{{ old('url', $district->url) }}" class="w-full border px-3 py-2 rounded">
                @error('url') <p class="text-red-500 font-semibold">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block font-semibold mb-1">Latitude</label>
                <input type="text" name="lat" value="{{ old('lat', $district->lat) }}" class="w-full border px-3 py-2 rounded">
                @error('lat') <p class="text-red-500 font-semibold">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block font-semibold mb-1">Longitude</label>
                <input type="text" name="lon" value="{{ old('lon', $district->lon) }}" class="w-full border px-3 py-2 rounded">
