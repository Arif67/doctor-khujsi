...
<div class="bg-white shadow rounded-lg p-6">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-semibold text-gray-800">Edit Area</h2>
    ...
    <form action="{{ route('admin.areas.update', $area->id) }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select name="district_id" id="district_id" class="w-full border px-3 py-2 rounded" required>
                    ...
                    @foreach($districts as $district)
                        <option value="{{ $district->id }}" @selected(old('district_id', $area->thana?->district_id) == $district->id)>{{ $district->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="thana_id" id="thana_id" class="w-full border px-3 py-2 rounded" {{ $thanas->isEmpty() ? 'disabled' : '' }} required>
                    <option value="">Select thana</option>
                    @foreach($thanas as $thana)
                        <option value="{{ $thana->id }}" @selected(old('thana_id', $area->thana_id) == $thana->id)>{{ $thana->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $area->name) }}" class="w-full border px-3 py-2 rounded" required>
                @error('name') <p class="text-red-500 font-semibold">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block font-semibold mb-1">Bangla Name</label>
                <input type="text" name="bn_name" value="{{ old('bn_name', $area->bn_name) }}" class="w-full border px-3 py-2 rounded">
                @error('bn_name') <p class="text-red-500 font-semibold">{{ $message }}</p> @enderror
            </div>
            <div class="md:col-span-2">
                <label class="block font-semibold mb-1">URL</label>
                <input type="text" name="url" value="{{ old('url', $area->url) }}" class="w-full border px-3 py-2 rounded">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const selectedThana = '{{ old('thana_id', $area->thana_id) }}';
    ...
    loadThanas(districtSelect.value, selectedThana);
});
</script>
@endpush
@endsection
